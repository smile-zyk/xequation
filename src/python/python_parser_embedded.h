#pragma once
// python_parser.py -- 内嵌为 C++ raw string，运行时用 pybind11::exec 加载。
// 与 python_parser.py 保持同步；编辑请改 python_parser.py 后重新生成。
static const char kPythonParserSource[] = R"PYSRC(import ast
import hashlib
...
class PythonParser:
    ...
    def _extract_dependencies(self, node, skip_names=None):
        ...
        return list(dict.fromkeys(dependencies)))PYSRC";
